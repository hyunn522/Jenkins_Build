...
- **개발자가 GitHub에 코드를 push**하면 → **Jenkins가 자동으로 빌드 & JAR 생성**
- 빌드된 JAR은 **Ubuntu 호스트와 Docker 컨테이너 간 Bind Mount**로 공유
- **inotifywait** 스크립트를 통해 JAR 갱신 시 자동 실행
- 완전한 **코드 → 빌드 → 실행 자동화** CI/CD 실습 구조 구축
...
2. **GitHub에 push**  
3. **GitHub Webhook → Jenkins 빌드 트리거**  
4. Jenkins → 코드 체크아웃 & 빌드(JAR 생성)  
5. 빌드 산출물 → **호스트 디렉토리 공유 (Bind Mount)**  
6. **호스트 감시 스크립트(inotifywait)** → JAR 변경 시 자동 실행  
...
즉, 개발자가 코드를 **GitHub에 push**하는 순간부터  
